...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      if (region?.id) {
        const zoneData = await getZones(region.id)
        setZones(zoneData)
      }

      if (zone?.id) {
        const woredaData = await getWoredas(zone.id)
        setWoredas(woredaData)
      }

      if (woreda?.id) {
        const kebeleData = await getKebeles(woreda.id)
  ...
  const handleRegionChange = async (value: string) => {
    ...
    const data = await getZones(value)
  ...
  const handleZoneChange = async (value: string) => {
    ...
    const data = await getWoredas(value)
  ...
  const handleWoredaChange = async (value: string) => {
    ...
    const data = await getKebeles(value)
            ...
            <Card className="p-7">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex justify-end">
                  ...
                  <Button onClick={handleUpdate} disabled={updating} className='bg-linear-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700'>
